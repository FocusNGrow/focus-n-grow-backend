...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!name || !email || !password) return res.status(400)
      .json({ status: 'error', message: 'Name, email and password required' });
    const existing = await User.findOne({ where: { email } });
    if (existing) return res.status(400)
      .json({ status: 'error', message: 'Email already registered' });
    const user = await User.create({ name, email, password,
      phone: phone || '', plan_type: 'parent' });
...
router.post('/link-child', async (req, res) => {
  try {
    ...
    const { data: link, error } = await sb()
      ...
      .eq('verified', false)
      .single();
    if (error || !link) return res.status(404)
      .json({ status: 'error',
        message: 'Invalid code. Ask your child to generate a new link code.' });
    await sb().from('parent_child_links')
      .update({ parent_user_id, verified: true }).eq('id', link.id);
    res.json({ status: 'success',
...
router.get('/:parent_id/children', async (req, res) => {
  try {
    const { data, error } = await sb().from('parent_child_links')
      .select().eq('parent_user_id', req.params.parent_id).eq('verified', true);
...
router.get('/child/:child_id/report', async (req, res) => {
  try {
    ...
    const child = await User.findOne({ where: { id: childId },
      attributes: ['id', 'name', 'email', 'plan_type', 'createdAt'] });
    if (!child) return res.status(404)
      .json({ status: 'error', message: 'Child not found' });
    const sevenDaysAgo = new Date(Date.now() - 7*24*60*60*1000).toISOString();
    const { data: moods } = await sb().from('school_mood_logs')
      .select().eq('student_user_id', childId).gte('logged_at', sevenDaysAgo)
      .order('logged_at', { ascending: false });
    const { data: completions } = await sb().from('assignment_completions')
      .select('*, assignments(subject, title, minimum_focus_minutes)')
      .eq('student_user_id', childId).eq('completed', true)
    ...
    res.json({ status: 'success', data: {
      child: { name: child.name, plan: child.plan_type },
      this_week: {
...
router.post('/generate-link-code', async (req, res) => {
  try {
    ...
    if (!child_user_id) return res.status(400)
      .json({ status: 'error', message: 'child_user_id required' });
    const linkCode = genLinkCode();
    await sb().from('parent_child_links')
      .delete().eq('child_user_id', child_user_id).eq('verified', false);
    const { data, error } = await sb().from('parent_child_links')
      .insert({ child_user_id, child_name: child_name || 'Student',
        link_code: linkCode }).select().single();
    if (error)