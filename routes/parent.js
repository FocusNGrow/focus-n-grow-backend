...
const getSecret = () =>
  process.env.JWT_SECRET || 'focus-n-grow-secret-key-2025';
...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await User.create({
      name, email, password, phone: phone || '', plan_type: 'parent' });
...
router.post('/link-child', async (req, res) => {
  try {
    ...
    if (error || !link) return res.status(404)
      .json({ status: 'error',
        message: 'Invalid code. Ask your child to generate a new one.' });
